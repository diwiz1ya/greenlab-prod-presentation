...
export function renderHero(counts = {}) {
  return `
    <section class="hero">
      <div>
        <div class="eyebrow">Готово к завтрашней встрече</div>
        <h1>Локальный рабочий поток: достаточно узкий для демо и понятный для обсуждения.</h1>
        <p class="lead">
          Задача этой версии не копировать весь CleanCloud, а показать внутренний
          операционный слой: корзины, QR-переходы, доступ по станциям и видимую
          границу синхронизации.
        </p>
      </div>
      <div class="grid">
